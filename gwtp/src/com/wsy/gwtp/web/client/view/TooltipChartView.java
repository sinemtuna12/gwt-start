...
import com.wsy.gwtp.web.client.model.Data;
import com.wsy.gwtp.web.client.model.TestData;

public class TooltipChartView extends ViewImpl implements
		TooltipChartPresenter.MyView {
	...
	public Widget asWidget() {
		if(panel == null) {
			...
			final ListStore<Data> pieStore = new ListStore<Data>(
					dataAccess.nameKey());
			...
			series.addSeriesItemOverHandler(new SeriesItemOverHandler<Data>() {
				...
				public void onSeriesOverItem(SeriesItemOverEvent<Data> event) {
					if (pieChart.getElement() != null) {
						...
						pieStore.clear();
						pieStore.add(new Data("data1", dataAccess.data1().getValue(
								store.get(index)), 0, 0, 0, 0, 0, 0, 0, 0));
						pieStore.add(new Data("data2", dataAccess.data2().getValue(
								store.get(index)), 0, 0, 0, 0, 0, 0, 0, 0));
						pieStore.add(new Data("data3", dataAccess.data3().getValue(
								store.get(index)), 0, 0, 0, 0, 0, 0, 0, 0));
						pieStore.add(new Data("data4", dataAccess.data4().getValue(
								store.get(index)), 0, 0, 0, 0, 0, 0, 0, 0));
						pieStore.add(new Data("data5", dataAccess.data5().getValue(
								store.get(index)), 0, 0, 0, 0, 0, 0, 0, 0));
